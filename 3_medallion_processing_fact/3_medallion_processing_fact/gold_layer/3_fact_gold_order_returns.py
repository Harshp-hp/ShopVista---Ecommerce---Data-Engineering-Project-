...
# 1) Calculate return_days = return_ts - order_dt
...
# add date id
df_union = df_union.withColumn("date_id", F.date_format(F.col("order_dt"), "yyyyMMdd").cast(IntegerType()))  # Create date_key
...
spark.sql(f"SELECT COUNT(*) FROM {catalog_name}.gold.gld_fact_order_returns").show()
