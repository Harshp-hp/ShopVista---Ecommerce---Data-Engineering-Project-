...
# Transformation :- drop deduplicates 

df = df.dropDuplicates(["shipment_id", "order_id"])

# Transformation : Convert 'order_dt' column to DateType
df = df.withColumn("order_dt", F.col("order_dt").cast("Date"))

# Transformation : Convert "reason" to upper case and trim whitespaces 
df = df.withColumn("carrier", F.upper(F.trim("carrier")))
...
def upsert_to_silver(microBatchDF, batchId):
    ...
    if not spark.catalog.tableExists(table_name):
        ...
        deltaTable.alias("silver_table").merge(
            ...
            "silver_table.transaction_id = batch_table.transaction_id and silver_table.shipment_id = batch_table.shipment_id",
...
spark.sql(f"SELECT COUNT(*) FROM {catalog_name}.silver.slv_order_shipments").show()
